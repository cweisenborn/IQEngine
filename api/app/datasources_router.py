    ...
    if feature_flags:
        ...
        if configuration.feature_flags.get("allowRefreshing", False):
            ...
            for datasource in all_datasources_list:
                ...
                background_tasks.add_task(datasources.sync, datasource["account"], datasource["container"], datasource.get("awsAccessKeyId", None))
        else:
...
async def sync_datasource(
    ...
    aws_access_key_id = existing_datasource.get("awsAccessKeyId", None)
    background_tasks.add_task(datasources.sync, account, container, aws_access_key_id)
